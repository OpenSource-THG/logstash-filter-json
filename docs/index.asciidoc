...
=== THG JSON filter plugin

include::{include_path}/plugin_header.asciidoc[]

==== Description

This is a special JSON parsing filter forked from the original JSON filter.
It takes an existing field which contains JSON and expands it into an
actual data structure within the Logstash event. It also has the extended
ability to replace characters in the keys of the object with other characters.
...
| <<plugins-{type}s-{plugin}-key_replacements>> |<<hash,hash>>|No
|=======================================================================
...
[id="plugins-{type}s-{plugin}-key_replacements"]
===== `key_replacements` 

  * Value type is <<hash,hash>>
  * There is no default value for this setting.

Define the characters to be replaced in the keys and what they should be
replace with

For example, if you want to replace the square brackets with round brackets,
use the following:
[source,ruby]
    filter {
      json {
...
This is to get round limitations of Logstash not being able to use square brackets in the keys of objects.
